...
bool RectangleIntersection::printRectangles() const
{
    if (error_flag)
        return false;
    std::cout << "Input:\n";
    for (size_t rectCount{}; rectCount < rectangles.size(); ++rectCount)
    {
        std::cout << rectCount + 1 << ": Rectangle at (" << rectangles[rectCount].x << "," << rectangles[rectCount].y
                  << "), w=" << rectangles[rectCount].w << ", h=" << rectangles[rectCount].h << ".\n";
    }
    std::cout << "\n";
    return true;
}
...
std::vector<std::pair<std::set<int>, Rectangle>> RectangleIntersection::computeIntersections() const
{
    ...
    for (size_t intersectCount{}; intersectCount < intersections.size(); ++intersectCount)
    {
        const auto [contributors, intersect] = intersections[intersectCount];
        for (size_t rectCount{}; rectCount < rectangles.size(); ++rectCount)
        {
            if (contributors.find(static_cast<int>(rectCount + 1)) == contributors.end())
            {
                Rectangle newIntersect = intersect.intersection(rectangles[rectCount]);
                if (newIntersect.isValid())
                {
                    std::set<int> newContributors{contributors};
                    newContributors.insert(static_cast<int>(rectCount + 1));
